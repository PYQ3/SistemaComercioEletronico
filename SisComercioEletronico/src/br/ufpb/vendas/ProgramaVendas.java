...
public class ProgramaVendas {
		...
		while (!sair) {
			...
			switch(opcao) {
			  ...
			  case 4:
				...
				List<Cliente> nomeDoClientePesq = sistema.pesquisarClientePeloNome(nomePesquisar);
			  ...
			  case 10:
					...
					try {
						gravarDados(sistema);
						gravarProdutos(sistema);
						gravarPedidos(sistema);
					} catch (IOException e) {
	...
	public static void gravarPedidos(SistemaComercioEletronico sistema) throws IOException{
		GravadorDeDados gravadorDePedidos = new GravadorDeDados();
		List<Pedido> pedidos = sistema.getPedidos();
		List<String> pedidosCadastrados = new ArrayList<String>();
		
		for (Pedido c : pedidos) {
			String linha = c.getData() + "#" + c.getStatus() + "#" + c.getNumero() + "#" + c.getFormaPagamento() + "#" + c.getItens() + "#" + c.getCliente();
			pedidosCadastrados.add(linha);
		}
		gravadorDePedidos.gravaTextoEmArquivo(pedidosCadastrados, "pedidos.txt");
	}
	
	public static void recuperarPedidos(SistemaComercioEletronico sistema) throws IOException{
		GravadorDeDados gravador = new GravadorDeDados();
		List<Pedido> pedidos = new ArrayList<Pedido>();
		List<String> pedidosCadastrados = gravador.recuperaTextoEmArquivo("pedidos.txt");
		for (String pedido : pedidosCadastrados) {
			String[] dados = pedido.split("#");
			Pedido c = new Pedido(dados[0], dados[1], dados[2], dados[3], Arrays.asList(dados[4]), Cliente.class.cast(dados[5]));
			pedidos.add(c);
		}
		sistema.setPedidos(pedidos);
		
		
	}
	
	public static void gravarProdutos(SistemaComercioEletronico sistema) throws IOException{
		GravadorDeDados gravadorDeProdutos = new GravadorDeDados();
		List<Produto> produtos = sistema.getProdutos();
		List<String> produtosCadastrados = new ArrayList<String>();
		
		for (Produto c : produtos) {
			String linha = c.getCodigo() + "#" + c.getDescricao() + "#" + c.getPrecoVenda() + "#" + c.getQuantidadeEmEstoque() + "#" + c.getCategoria();
			produtosCadastrados.add(linha);
		}
		gravadorDeProdutos.gravaTextoEmArquivo(produtosCadastrados, "produtos.txt");
	}

	public static void recuperarProdutos(SistemaComercioEletronico sistema) throws IOException{
		GravadorDeDados gravador = new GravadorDeDados();
		List<Produto> produtos = new ArrayList<Produto>();
		List<String> produtosCadastrados = gravador.recuperaTextoEmArquivo("produtos.txt");
		for (String produto : produtosCadastrados) {
			String[] dados = produto.split("#");
			Produto c = new Produto(dados[0], dados[1], Double.parseDouble(dados[2]), Integer.parseInt(dados[3]), dados[4]);
			produtos.add(c);
		}
		sistema.setProdutos(produtos);
